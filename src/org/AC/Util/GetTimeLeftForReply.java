...
public class GetTimeLeftForReply {
	private static final Logger logger = Logger
			.getLogger(GetTimeLeftForReply.class);
		...
		try {
			if (bookingTime != null) {
				...
				if (date1.compareTo(date2) >= 0) {
					...
					double diffInDays = (double) ((date1.getTime() - date2
					...
					if (diffInDays != 0.0) {
						...
						if (c.get(Calendar.HOUR_OF_DAY)
								- mbCal.get(Calendar.HOUR_OF_DAY) < 0) {
							long hours = c.get(Calendar.HOUR_OF_DAY)
									- mbCal.get(Calendar.HOUR_OF_DAY);
							hours = hours + 24;
							time.setHours(hours);
						...
						} else if (c.get(Calendar.MINUTE)
								- mbCal.get(Calendar.MINUTE) == 0) {
							time.setMinutes(0);
						} else {
							time.setMinutes(c.get(Calendar.MINUTE)
									- mbCal.get(Calendar.MINUTE));
						}
					}
					list.add(time);
				}
			...
			logger.error("getHoursAndMinutes method of GetTimeLeftForReply threw error:"
					+ e.getMessage());
			e.printStackTrace();
		}
