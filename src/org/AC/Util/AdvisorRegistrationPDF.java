...
public class AdvisorRegistrationPDF {
	private static final Logger logger = Logger.getLogger(AdvisorRegistrationPDF.class);
	...
	private static Font titleFont = new Font(Font.FontFamily.TIMES_ROMAN, 18,
		      Font.UNDERLINE);
	public void createPDF(HttpServletResponse response,List<AdvisorProfileDTO> advisor,List<ProfessionalBackgroundDTO> professionalBackground,
						List<AdvisorServiceDTO> services,List<AdvisorModeDTO> modes, List<AdvisorEducationDTO> education,List<AdvisorProfileDTO> keySkills
						,List<AdvisorProfileDTO> awards){
	    ...
	    try {
			...
			logger.error("createPDF method of AdvisorRegistrationPDF threw error:"+e.getMessage());
			e.printStackTrace();
		} catch (DocumentException e) {
			logger.error("createPDF method of AdvisorRegistrationPDF threw error:"+e.getMessage());
			e.printStackTrace();
		}catch (IOException e1) {
			logger.error("createPDF method of AdvisorRegistrationPDF threw error:"+e1.getMessage());
			e1.printStackTrace();
		}
	}
	...
	private static void addTitlePage(Document document)
	 ...
	 private static void addEducationInfo(Document document,List<AdvisorEducationDTO> education) {
		 ...
		 logger.info("Exit addEducationInfo method of AdvisorRegistrationPDF");
			...
			logger.error("addEducationInfo method of AdvisorRegistrationPDF threw error:"+e.getMessage());
			e.printStackTrace();
		}
	 }
	 
	 private static void addGeneralInfo(Document document,List<AdvisorProfileDTO> advisor) {
		 ...
		 try {
			 ...
			 for (AdvisorProfileDTO profile : advisor) {
				 ...
				 preface.add(new Paragraph("Email :"+profile.getEmail()+"\n", catFont));
		 ...
		 logger.info("Exit addGeneralInfo method of AdvisorRegistrationPDF");
			...
			logger.error("addGeneralInfo method of AdvisorRegistrationPDF threw error:"+e.getMessage());
			e.printStackTrace();
		}
	 }	
	 
	 private static void addOtherInfo(Document document,List<AdvisorProfileDTO> keySkills,List<AdvisorProfileDTO> awards,List<AdvisorProfileDTO> advisor) {
		 ...
		 try {
			 ...
			 for (AdvisorProfileDTO profile : awards) {
				 preface.add(new Paragraph("Achievements and Awards :"+profile.getAchievements()+"\n", catFont));
			}
			 ...
			 for (AdvisorProfileDTO hobby : advisor) {
				 preface.add(new Paragraph("Know Your Advisor Better(Hobbies & Interests) :"+hobby.getKnowYourAdvisor()+"\n", catFont));
			}
			 document.add(preface);
		 logger.info("Exit addOtherInfo method of AdvisorRegistrationPDF");
		} catch (DocumentException e) {
			logger.error("addOtherInfo method of AdvisorRegistrationPDF threw error:"+e.getMessage());
			e.printStackTrace();
		}
	 }
	 
	 private static void addProfessionalBackground(Document document ,List<ProfessionalBackgroundDTO> prof,List<AdvisorProfileDTO> advisor){
		 ...
		 try {
			 if(prof.size() > 0) {
				 ...
				 for (AdvisorProfileDTO adv : advisor) {
					 preface.add(new Paragraph("Experience :"+adv.getExperience()+ "\n", catFont));
				}
				 for (ProfessionalBackgroundDTO obj : prof) {
					 ...
					 for (String description  : obj.getDescriptionArray()) {
						 if(description != null && !("").equals(description)){
							 preface.add(new Paragraph("Description :"+description+ "\n", catFont));
						 }
					}
				}
				 document.add(preface);
			 ...
			 logger.info("Exit addProfessionalBackground method of AdvisorRegistrationPDF");
				...
				logger.error("addProfessionalBackground method of AdvisorRegistrationPDF threw error:"+e.getMessage());
				e.printStackTrace();
			}catch (Exception e) {
	 ...
	 private static void addServicesAndModes(Document document,List<AdvisorServiceDTO> services,List<AdvisorModeDTO> modes){
		 ...
		 try {
				 ...
				 for (AdvisorServiceDTO service : services) {
					 ...
					 for (AdvisorModeDTO mode : modes) {
						 if(service.getAdvisorId() == mode.getAdvisorId()){
							 preface.add(new Paragraph("Mode :"+mode.getModeOfCommunication()+ "\n", catFont)); 
							 preface.add(new Paragraph("Price :"+mode.getPrice()+ "\n", catFont)); 
			 ...
			 logger.info("Exit addServicesAndModes method of AdvisorRegistrationPDF");
				...
				logger.error("addServicesAndModes method of AdvisorRegistrationPDF threw error:"+e.getMessage());
				e.printStackTrace();
			}catch (Exception e) {
				logger.error("addServicesAndModes method of AdvisorRegistrationPDF threw error:"+e.getMessage());
				e.printStackTrace();
	 ...
	 private static void addImage(Document document,List<AdvisorProfileDTO> advisor){
		 ...
		 try {
			 for (AdvisorProfileDTO advisorProfileDTO : advisor) {
				 if(!advisorProfileDTO.getImage().isEmpty()) {
				        ...
				        Image image2 = Image.getInstance(advisorProfileDTO.getImage());
			 ...
			 logger.info("Exit addImage method of AdvisorRegistrationPDF");
				...
				logger.error("addImage method of AdvisorRegistrationPDF threw error:"+e.getMessage());
				e.printStackTrace();
			}catch (Exception e) {
				logger.error("addImage method of AdvisorRegistrationPDF threw error:"+e.getMessage());
				e.printStackTrace();
