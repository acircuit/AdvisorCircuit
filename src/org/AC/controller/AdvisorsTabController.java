...
public class AdvisorsTabController extends HttpServlet {
	...
	private static final Logger logger = Logger
			.getLogger(AdvisorsTabController.class);

	/**************************************
	 * COMMENTS*************************************************** This class
	 * will get all the advisor based upon services chosen from the navbar.
	 * 
	 * @return :None
	...
	protected void doGet(HttpServletRequest request,
		...
		if (("CVCritique").equals(service)) {
			// Get the Details of all the advisor who are offering CVCritique as
			// a service
			...
			// Get the Details of all the advisor who are offering MockInterview
			// as a service
			String services = "mockinterview";
			SearchDAO searchForCV = new SearchDAO();
			advisorsByServices = searchForCV.getAdvisorsByService(services);
		} else if (("PersonalWorkshop").equals(service)) {
			// Get the Details of all the advisor who are offering
			// PersonalWorkshop as a service
			String services = "personalworkshops";
			SearchDAO searchForCV = new SearchDAO();
			advisorsByServices = searchForCV.getAdvisorsByService(services);
		} else if (("CareerTalk").equals(service)) {
			// Get the Details of all the advisor who are offering CarrerTalk as
			// a service
		 ...
		 * profession) { if(professionalBackgroundDTO.getCompany().length() >=
		 * 19){
		 * professionalBackgroundDTO.setCompany(professionalBackgroundDTO.getCompany
		 * ().substring(0, 18)); }
		 * if(professionalBackgroundDTO.getDesignation().length() >= 19){
		 * professionalBackgroundDTO
		 * .setDesignation(professionalBackgroundDTO.getDesignation
		 * ().substring(0, 18)); } }
		 */
