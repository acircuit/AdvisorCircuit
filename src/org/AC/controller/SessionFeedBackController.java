...
public class SessionFeedBackController extends HttpServlet {
	...
	private static final Logger logger = Logger
			.getLogger(SessionFeedBackController.class);
	...
	protected void doPost(HttpServletRequest request,
		...
		InputStream resourceAsStream1 = Thread.currentThread()
				.getContextClassLoader()
				.getResourceAsStream("Resources/mail.properties");
		prop1.load(resourceAsStream1);
		if (sId != null && body != null && subject != null && type == null
				&& advisor == null && reject == null && email == null) {
			...
			if (!isFeedBackGiven) {
				...
				Boolean isFeedbackCommit = feedback.SetUserFeedBack(sId, body,
						subject, fileURL);
				if(isFeedbackCommit){
					 int[] ids = new int[3];
					 ...
					 String content = "Hi, <br><br>A Follow Up Mail has been sent by the User to the Advisor for: <br>Session Id : " +sId+ "<br>Given By : USER"+"<br><img src=\"https://www.advisorcircuit.com/Test/assets/img/logo_black.png\" style='float:right' width='15%'>";
				...
				if (again != null) {
					...
					prop.load(resourceAsStream);
					String path = prop
							.getProperty("SESSION_FEEDBACK_FILE_PATH");
					Part filePart = request.getPart("myFile");
					String fileName = getFileName(filePart);
					String fileURL = "";
					if (fileName != null) {
					...
					Boolean isFeedbackCommit = feedback.SetUserFeedBackAgain(
							sId, body, subject, fileURL);
					...
					if(isFeedbackCommit){
						 ...
						 String content = "Hi, <br><br>A Follow Up Mail has been sent by the User to the Advisor after rejection for: <br>Session Id : " +sId+ "<br>Given By : USER"+"<br><img src=\"https://www.advisorcircuit.com/Test/assets/img/logo_black.png\" style='float:right' width='15%'>";
						 SendMail mail = new SendMail(subjects, content, prop1.getProperty("MAIL_ADMIN"),prop1.getProperty("MAIL_ADMIN"));
						 mail.start();
						 response.getWriter().write("<p style='color: #c84c4e'>Your Follow Up Mail has been sent</p>");
					}
					...
					response.getWriter()
							.write("<p style='color: #c84c4e'>Your Follow Up Mail has already been sent</p>");
				}
			}
		} else if (sId != null && type != null && reject == null
				&& again == null && email == null) {
		...
		} else if (sId != null && body != null && subject != null
				&& advisor != null && advisor.equals("true") && reject == null
				&& again == null && email == null) {
			Properties prop = new Properties();
			InputStream resourceAsStream = Thread.currentThread()
					.getContextClassLoader()
			...
			// Delete previous
			SessionFeedBackDAO feedback = new SessionFeedBackDAO();
			Boolean isFeedbackCommit = feedback.SetAdvisorFeedBack(sId, body,
					subject, fileURL);
			...
			if(isFeedbackCommit){
				...
				admin.InsertNotification(comment, href);
				 ...
				 String content = "Hi, <br><br>A Follow Up Mail has been sent by the Advisor to the User for:<br>Session Id : " +sId+ "<br>Given By : ADVISOR"+"<br><img src=\"https://www.advisorcircuit.com/Test/assets/img/logo_black.png\" style='float:right' width='15%'>";
				 SendMail mail = new SendMail(subjects, content, prop1.getProperty("MAIL_ADMIN"),prop1.getProperty("MAIL_ADMIN"));
				 mail.start();
				 response.getWriter().write("<p style='color: #c84c4e'>Your Feedback has been sent</p>");
			}

		} else if (sId != null && action != null && reject == null
				&& again == null && email == null) {
			...
			if (!isFeedBackGiven) {
				...
				Boolean isFeedbackCommit = feedback.SetUserEmail(sId, body,
						subject, fileURL);
				if(isFeedbackCommit){
					 ...
					 String content = "Hi, <br><br>A session mail has been recieved for: <br>Session Id : " +sId+ "<br>Given By : USER"+"<br><img src=\"https://www.advisorcircuit.com/Test/assets/img/logo_black.png\" style='float:right' width='15%'>";
					 SendMail mail = new SendMail(subjects, content, prop1.getProperty("MAIL_ADMIN"),prop1.getProperty("MAIL_ADMIN"));
					 mail.start();
				...
				if (again != null && again.equals("true")) {
					...
					SessionFeedBackDAO feedback = new SessionFeedBackDAO();
					Boolean isFeedbackCommit = feedback.SetUserMailAgain(sId,
							body, subject, fileURL);
					if(isFeedbackCommit){
						...
						admin.InsertNotification(comment, href);
						 ...
						 String content = "Hi, <br><br>A session Mail has been recieved again after rejection for: <br>Session Id : " +sId+ "<br>Given By : USER"+"<br><img src=\"https://www.advisorcircuit.com/Test/assets/img/logo_black.png\" style='float:right' width='15%'>";
						 SendMail mail = new SendMail(subjects, content, prop1.getProperty("MAIL_ADMIN"),prop1.getProperty("MAIL_ADMIN"));
						 mail.start();
						 response.getWriter().write("<p style='color: #c84c4e'>Your Mail has been sent</p>");
					}
					...
					response.getWriter().write(
							"Your Mail has already been sent");
				}
			}
		} else if (sId != null && body != null && subject != null
				&& email != null && email.equals("advisor")) {
			Properties prop = new Properties();
			InputStream resourceAsStream = Thread.currentThread()
					.getContextClassLoader()
					.getResourceAsStream("Resources/Path.properties");
			...
			// Delete previous
			SessionFeedBackDAO feedback = new SessionFeedBackDAO();
			Boolean isFeedbackCommit = feedback.SetAdvisorMail(sId, body,
					subject, fileURL);
			if(isFeedbackCommit){
				...
				admin.InsertNotification(comment, href);
				 ...
				 String content = "Hi, <br><br>A session mail reply has been recieved for:<br>Session Id : " +sId+ "<br>Given By : ADVISOR"+"<br><img src=\"https://www.advisorcircuit.com/Test/assets/img/logo_black.png\" style='float:right' width='15%'>";
				 SendMail mail = new SendMail(subjects, content, prop1.getProperty("MAIL_ADMIN"),prop1.getProperty("MAIL_ADMIN"));
				 mail.start();
				 response.getWriter().write("<p style='color: #c84c4e'>Your Mail has been sent</p>");
			}
			...
			if (isApprove.equals("true")) {
				// update the user feedback status as approved
				SessionFeedBackDAO approve = new SessionFeedBackDAO();
				Boolean isStatusCommit = approve.SetApproveMailStatus(sId,
						target);
				if(isStatusCommit){
					int[] ids = new int[3];
					//Get Advisor Id
					...
					if(target.equals("user")){
						...
						String comment = "You've got a session mail reply from "+advisorName;
						String userHref = "UserUpcomingSessionViewDetails?rId="+ids[2];
						UserNotificationDAO user = new UserNotificationDAO();
				...
				Boolean isStatusCommit = rejected.SetRejectMailStatus(sId,
						target);
				if(isStatusCommit){
	...
	private String getFileName(final Part part) {
		...
		try {
			...
			logger.error("getFileName method of SetFile threw error:"
					+ e.getMessage());
			e.printStackTrace();
		}
		return null;
