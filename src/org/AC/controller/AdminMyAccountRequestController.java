...
public class AdminMyAccountRequestController extends HttpServlet {
	...
	private static final Logger logger = Logger
			.getLogger(AdminMyAccountRequestController.class);

	/**************************************
	 * COMMENTS*************************************************** This method
	 * will fetch all the request sent to the advisor to show on the
	 * adminrequests.jsp
	 * 
	 * @return :None
		...
		if (isError != null && !isError) {
			...
			for (UserRequestDTO userRequestDTO : requestList) {
				SimpleDateFormat dateFormat = new SimpleDateFormat(
						"dd-MMM-yyyy' 'h:mm a");
				userRequestDTO.setBookingTimeString(dateFormat
						.format(userRequestDTO.getBookingTime()));
				...
				if (pending != null && ("true").equals(pending)) {
					if (("PENDING FOR ADMIN APPROVAL").equals(status)) {
						...
						if (difference.size() > 0) {
							...
							/*
							 * //If advisor has lapsed the the time to reply,
							 * then change the status of the request. String
							 * status1 = "SESSION LAPSE BY ADVISOR";
							 * ChangeRequestStatusDAO change = new
							 * ChangeRequestStatusDAO(); isStatusCommit =
							 * change.setStatus(status1,requestId);
			...
			if (advisorIds.size() > 0) {
				...
				advisorDetails = advisorDetail
						.getAdvisorDetailsUsingAdvisorId(advisorIds);
			}
