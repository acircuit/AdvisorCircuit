		...
		if(isError!= null &&  !isError){
		    if(rId != null && !("").equals(rId)){
				...
				if(("REQUEST ACCEPTED BY ADVISOR WITH NEW DATES").equals(requestStatus) || ("REQUEST ACCEPTED BY ADVISOR").equals(requestStatus)){
					...
					System.out.println(sessionDetail.size());
					if(("REQUEST ACCEPTED BY ADVISOR WITH NEW DATES").equals(requestStatus)){
						...
						if(sessionId != 0){
							...
							if(advisorNewDates.size() > 0){
								for (AdvisorNewDatesDTO advisorNewDatesDTO : advisorNewDates) {
									if(advisorNewDatesDTO.getNewDate1() != null) {
										advisorNewDatesDTO.setNewDateString1(new SimpleDateFormat("dd-MMM-yyyy' 'h:mm a").format(new Date(advisorNewDatesDTO.getNewDate1().getTime())));
									}
									if(advisorNewDatesDTO.getNewDate2() != null) {
										advisorNewDatesDTO.setNewDateString2(new SimpleDateFormat("dd-MMM-yyyy' 'h:mm a").format(new Date(advisorNewDatesDTO.getNewDate2().getTime())));
									}
									if(advisorNewDatesDTO.getNewDate3() != null) {
										advisorNewDatesDTO.setNewDateString3(new SimpleDateFormat("dd-MMM-yyyy' 'h:mm a").format(new Date(advisorNewDatesDTO.getNewDate3().getTime())));
									}
								}
							}
						}
						
					}else if (("REQUEST ACCEPTED BY ADVISOR").equals(requestStatus)) {
						for (SessionDTO sessionDTO : sessionDetail) {
