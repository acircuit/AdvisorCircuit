...
public class UserMyAccountCancelledSessionController extends HttpServlet {
	...
	private static final Logger logger = Logger
			.getLogger(UserMyAccountCancelledSessionController.class);
	...
	protected void doGet(HttpServletRequest request,
		...
		if (username != null && userId != 0 && !("").equals(username)) {
			...
			list1 = requestDetails.getUserRequestDetailsUsingUserId(
			...
			for (UserRequestDTO userRequestDTO : list1) {
				...
				if (userRequestDTO.getStatus().equals(
						"REQUEST REJECTED BY ADMIN")) {
					userRequestDTO
							.setStatus("YOUR REQUEST COULD NOT BE ACCEPTED");
				}
				if (counter == 0) {
