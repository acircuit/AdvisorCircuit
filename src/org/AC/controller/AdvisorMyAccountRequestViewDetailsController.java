		...
		if(rId != null && !("").equals(rId)){
			...
			if(("REQUEST ACCEPTED BY ADVISOR WITH NEW DATES").equals(status) || ("REQUEST ACCEPTED BY ADVISOR").equals(status)){
				...
				for (SessionDTO sessionDTO : sessionDetails) {
					...
					if(sessionDTO.getAcceptedDate() != null && !("").equals(sessionDTO.getAcceptedDate())){
				...
				if(("REQUEST ACCEPTED BY ADVISOR WITH NEW DATES").equals(status)) {
					...
					for (AdvisorNewDatesDTO advisorNewDatesDTO : newDate) {
						if(mode.equals("email")){
							advisorNewDatesDTO.setNewDateString1(new SimpleDateFormat("dd-MMM-yyyy").format(new Date(advisorNewDatesDTO.getNewDate1().getTime())));
						}else{
